...
Manages server, CLI, perplexity, benchmarks, multimodal, installation, validation.
This edition is for RTX 4060+ users, with CUDA-optimized defaults and verification.
...
DEFAULTS = {
    "build_bin_dir": "",          # auto-detected or fallback
...
def find_llama_bin_dir():
    """Try to locate the build-cuda/bin/Release folder."""
    # First, check the default user's home install path
    ...
    # Look for any build-cuda* directory
        ...
        for d in os.listdir(base):
            if d.startswith("build-"):
                ...
                if os.path.isdir(bin_dir) and os.path.exists(os.path.join(bin_dir, "llama-cli.exe")):
                    return bin_dir
    # Fallback: check PATH for llama-cli.exe
    import shutil
...
VERIFY_SCRIPT = r"""
$LlamaDir = "$HOME\llama.cpp\build-cuda\bin\Release"
$testOps = Join-Path $LlamaDir "test-backend-ops.exe"
if (-not (Test-Path $testOps)) { Write-Host "FAIL: test-backend-ops not found"; exit 1 }
Write-Host "Testing CUDA backend..."
$out = & $testOps test -b cuda 2>&1 | Out-String
if ($LASTEXITCODE -eq 0 -and $out -match "All tests passed|OK") { Write-Host "CUDA backend: OK" } else { Write-Host "CUDA backend: FAIL"; exit 1 }
Write-Host "Testing CPU backend..."
$out = & $testOps test -b cpu 2>&1 | Out-String
if ($LASTEXITCODE -eq 0 -and $out -match "All tests passed|OK") { Write-Host "CPU backend: OK" } else { Write-Host "CPU backend: FAIL"; exit 1 }
...
class LlamaGuiApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Llama.cpp Control Center (RTX 4060+ Edition) - by Shashin Somanayake")
    ...
    def setup_ui(self):
        ...
        self.verify_btn = ttk.Button(status_frame, text="✅ Verify", command=self.run_verify_ps1)
    ...
    def run_install_ps1(self):
        if messagebox.askyesno("Install llama.cpp", "This will download and build llama.cpp with CUDA. Continue?"):
            try:
                ...
                # After install, rescan
                self.root.after(10000, self.update_status)  # wait 10 secs for install to finish
            except Exception as e:
